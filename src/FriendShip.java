...
public class FriendShip {
    ...
    public void makeFriend(String nameKey, String friendName) {
        ...
        if(!friends.isEmpty())
            for(int i =0;i<friends.size();i++){
                if(!friends.get(i).nameKey.contains(nameKey) &&  friends.get(i).friendName.contains(nameKey) ) {
    ...
    public void unmakeFriend(String nameKey, String friendName) {
        if(!friends.isEmpty())
        {
            for(int i=0;i<friends.size();i++)
            {
                if(friends.get(i).nameKey.equals(nameKey) && friends.get(i).friendName.equals(friendName))
                    friends.remove(i);
            }
        }
    }
    ...
    public List<String> getIndirectFriends(String nameKey) {
        ...
        if(!friends.isEmpty() )
            for(int i=0;i<friends.size();i++)
                if( friends.get(i) !=null &&  friends.get(i).nameKey.equals(nameKey))
                    fri.add(friends.get(i).friendName);
        ...
        if(!fri.isEmpty())  {
            ...
            for(int i =0;i<fri.size();i++){
                ...
                for(int j=0;j<friends.size();j++)
                    if( friends.get(j) !=null &&  friends.get(j).nameKey.equals(s))
                        all.add(friends.get(j).friendName);
